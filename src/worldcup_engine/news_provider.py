...
def fetch_live_team_news(team_name) -> str:
    ...
    by returning an empty string. Uses an in-memory cache to avoid duplicate calls.
    """
    normalized_name = team_name.lower().strip()
    if normalized_name in _news_cache:
        return _news_cache[normalized_name]
        
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(_provider.fetch_news, team_name)
        try:
            res = future.result(timeout=2.0)
            _news_cache[normalized_name] = res
            return res
